...
.NOTES
    ...
    Permissions needed: User.Read.All, Directory.Read.All, Organization.Read.All, Mail.Read, Files.Read.All
#>
...
try {
    ...
    Connect-MgGraph -Scopes "User.Read.All", "Directory.Read.All", "Organization.Read.All", "Mail.Read", "Files.Read.All" -NoWelcome
...
# Get all mailboxes information (using beta endpoint for RecipientTypeDetails)
Write-Host "Retrieving mailbox information..." -ForegroundColor Cyan
$mailboxes = @{}
try {
    $mailboxList = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/v1.0/users?`$select=id,mail,userPrincipalName&`$filter=mail ne null" -OutputType PSObject
    foreach ($mbx in $mailboxList.value) {
        $mailboxes[$mbx.id] = $mbx
    }
} catch {
...
foreach ($user in $users) {
    ...
    # Determine mailbox type with enhanced detection
    $mailboxType = "No Mailbox"
    if ($user.Mail -or $mailboxes.ContainsKey($user.Id)) {
        try {
            # Try to get mailbox details from Graph API
            $mailboxInfo = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/users/$($user.Id)?`$select=mailboxSettings" -OutputType PSObject -ErrorAction SilentlyContinue
            
            # Check recipient type via assigned licenses and properties
            if ($user.UserType -eq "Guest") {
                $mailboxType = "Guest Mailbox"
            } else {
                # Default to User Mailbox if they have mail
                $mailboxType = "User Mailbox"
                
                # Check for shared mailbox indicators (no license requirement, shared via delegates)
                # Note: Precise detection requires Exchange Online cmdlets, this is a best-effort approach
            }
        } catch {
