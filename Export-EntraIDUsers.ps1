...
$users = Get-MgUser -All -Property @(
    ...
    'AccountEnabled', 'OnPremisesSyncEnabled', 'EmployeeType', 'UserType', 'AssignedLicenses'
)
...
$licenseNames = @{
    'O365_BUSINESS_ESSENTIALS' = 'Office 365 Business Essentials'; 'O365_BUSINESS_PREMIUM' = 'Office 365 Business Premium'
    'DESKLESSPACK' = 'Office 365 F3'; 'DESKLESSWOFFPACK' = 'Office 365 F3'; 'ENTERPRISEPACK' = 'Office 365 E3'
    'ENTERPRISEPREMIUM' = 'Office 365 E5'; 'ENTERPRISEPREMIUM_NOPSTNCONF' = 'Office 365 E5 Without Audio Conferencing'
    'SPE_E3' = 'Microsoft 365 E3'; 'SPE_E5' = 'Microsoft 365 E5'; 'MICROSOFT365_F1' = 'Microsoft 365 F1'
    'MICROSOFT365_F3' = 'Microsoft 365 F3'; 'EXCHANGESTANDARD' = 'Exchange Online Plan 1'
    'EXCHANGEENTERPRISE' = 'Exchange Online Plan 2'; 'POWER_BI_STANDARD' = 'Power BI Free'
    'POWER_BI_PRO' = 'Power BI Pro'; 'PROJECTPROFESSIONAL' = 'Project Plan 3'; 'PROJECTESSENTIALS' = 'Project Plan 1'
    'VISIOCLIENT' = 'Visio Plan 2'; 'TEAMS_EXPLORATORY' = 'Microsoft Teams Exploratory'; 'STREAM' = 'Microsoft Stream'
    'AAD_PREMIUM' = 'Azure Active Directory Premium P1'; 'AAD_PREMIUM_P2' = 'Azure Active Directory Premium P2'
    'INTUNE_A' = 'Microsoft Intune'; 'FLOW_FREE' = 'Microsoft Power Automate Free'
...
foreach ($user in $users) {
    ...
    $manager = $null
    try { $manager = Get-MgUserManager -UserId $user.Id -ErrorAction SilentlyContinue } catch { }
    ...
    if ($user.AssignedLicenses -and $user.AssignedLicenses.Count -gt 0) {
        foreach ($license in $user.AssignedLicenses) {
            if ($skuHashTable.ContainsKey($license.SkuId)) {
                ...
                $friendlyName = if ($licenseNames.ContainsKey($skuPartNumber)) { $licenseNames[$skuPartNumber] } else { $skuPartNumber }
    ...
    $exportData += [PSCustomObject]@{
        'Source' = $source; 'Employment Status' = if ($user.EmployeeType) { $user.EmployeeType } else { "Not Set" }
        'Display Name' = $user.DisplayName; 'Last Name' = $user.Surname; 'First Name' = $user.GivenName
        'UPN' = $user.UserPrincipalName; 'Title' = $user.JobTitle; 'Department' = $user.Department
        'Manager' = if ($manager) { $manager.AdditionalProperties.displayName } else { "" }
        'Email Address' = $user.Mail; 'Phone Number' = $phoneNumber; 'Physical Delivery Address' = $addressString
        'Language Preference' = $user.PreferredLanguage; 'Account Status' = $accountStatus
        'Blocked Credentials' = $blockedCredentials; 'Account Type' = $user.UserType
        'Mailbox Type' = $mailboxType; 'Has OneDrive' = $hasOneDrive; 'Assigned Licenses' = $licenseString
    }
}
